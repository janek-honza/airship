...
func _physics_process(delta):
	...
	if engine_speed <= target_engine_speed:
		engine_speed = min(engine_speed + delta, target_engine_speed)
	else:
		engine_speed = max(engine_speed - delta, target_engine_speed)
	...
	self.rotation.y = self.rotation.y + yaw_input * yaw_speed * (5 / (abs(relative_speed_forward)+1))
